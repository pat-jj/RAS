...
def upload_to_hub(args, model_path, repo_id, token):
    ...
    api.upload_file(
        ...
        path_in_repo=f"latest_checkpoint_epoch_{args.finetune_method}_{args.llm_frozen}_qwen_v3.safetensors",
        repo_id=repo_id,
    )
...
def setup_logging(args):
    ...
    log_file = os.path.join('planner/logs', f'training_qwen_{timestamp}.log')
...
def setup_wandb(args):
    ...
    wandb.init(
        project="planner-all-train-qwen",
        config=args,
        ...
        for batch_idx, batch in enumerate(progress_bar):
            try:
                ...
                if current_step in checkpoint_points:
                    ...
                    model_path = os.path.join(args.output_dir, f'checkpoint_{position}_of_20_qwen.safetensors')
                ...
                emergency_path = os.path.join(args.output_dir, 'emergency_checkpoint_qwen.safetensors')
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='/shared/rsaas/pj20/firas_data/multitask/checkpoints_qwen')
    ...
    for epoch in range(args.epochs):
        ...
        try:
            ...
            if val_loss < best_val_loss:
                ...
                model_path = os.path.join(args.output_dir, 'best_model_qwen.safetensors')
            ...
            model_path = os.path.join(args.output_dir, 'latest_checkpoint_qwen.safetensors')
            save_model(model, model_path)
